...
Image both_images(const Image& a, const Image& b) {
  ...
  for (int k = 0; k < both.c; ++k)
    for (int j = 0; j < a.h; ++j)
      for (int i = 0; i < a.w; ++i) both(i, j, k) = a(i, j, k);

  for (int k = 0; k < both.c; ++k)
    for (int j = 0; j < b.h; ++j)
      for (int i = 0; i < b.w; ++i) both(i + a.w, j, k) = b(i, j, k);
  return both;
}
...
Image find_and_draw_matches(const Image& a, const Image& b, float sigma,
  ...
  vector<Descriptor> ad =
      harris_corner_detector(a, sigma, thresh, window, nms, corner_method);
  vector<Descriptor> bd =
      harris_corner_detector(b, sigma, thresh, window, nms, corner_method);
...
// const vector<Descriptor>& a, b: array of descriptors for pixels in two
// images. returns: best matches found. For each element in a[] find the index
// of best match in b[]
vector<int> match_descriptors_a2b(const vector<Descriptor>& a,
  ...
  for (int j = 0; j < (int)a.size(); j++) {
    ...
    const Descriptor& a_desc = a[j];
    for (int i = 0; i < b.size(); i++) {
      auto& b_desc = b[i];
      const float distance = l1_distance(a_desc.data, b_desc.data);
...
// const vector<Descriptor>& a, b: array of descriptors for pixels in two
// images.
// returns: best matches found. each descriptor in a should match with
// at most one other descriptor in b.
vector<Match> match_descriptors(const vector<Descriptor>& a,
                                const vector<Descriptor>& b) {
  if (a.size() == 0 || b.size() == 0) return {};

  vector<Match> m;

  const std::vector<int> a_in_b = match_descriptors_a2b(a, b);
  const std::vector<int> b_in_a = match_descriptors_a2b(b, a);
  // Use match_descriptors_a2b(a,b) and match_descriptors_a2b(b,a)
  // and populate `m` with good matches!
  std::map<int, int> a_in_b_count;
  for (int i = 0; i < a_in_b.size(); i++) {
    a_in_b_count[a_in_b[i]] += 1;
  }
  std::map<int, int> b_in_a_count;
  for (int i = 0; i < b_in_a.size(); i++) {
    b_in_a_count[b_in_a[i]] += 1;
  }

  std::map<int, bool> a_matched;
  for (int i = 0; i < a.size(); i++) {
    // Check if the match is symmetric.
    if (b_in_a[a_in_b[i]] == i) {
      m.push_back(Match{&a[i], &b[a_in_b[i]],
                        l1_distance(a[i].data, b[a_in_b[i]].data)});
    }
  }
  return m;
}
...
Point project_point(const Matrix& H, const Point& p) {
  ...
  Matrix homogeneous_coordinates(3, 1);
  homogeneous_coordinates(0, 0) = p.x;
  homogeneous_coordinates(1, 0) = p.y;
  homogeneous_coordinates(2, 0) = 1;

  Matrix projection = H * homogeneous_coordinates;
  Matrix final_coordinates = projection / projection(2, 0);
  return Point(final_coordinates(0, 0), final_coordinates(1, 0));
}
...
// returns: inliers whose projected point falls within thresh of their match in
// the other image.
vector<Match> model_inliers(const Matrix& H, const vector<Match>& m,
                            float thresh) {
...
void randomize_matches(vector<Match>& m) {
  ...
  for (int i = m.size() - 1; i > 0; i--) {
    int rand_idx = arc4random() % (i + 1);
    Match temp = m[rand_idx];
    m[rand_idx] = m[i];
    m[i] = temp;
  }
}
...
Matrix RANSAC(vector<Match> m, float thresh, int k, int cutoff) {
  ...
  for (int i = 0; i < k; i++) {
    randomize_matches(m);
    // Pick the minimum number of matches needed for calcualting homography,
    // i.e. 4
    std::vector<Match> trimmed_matches(m.begin(), m.begin() + 4);
    // Compute a homography with a few matches.
    Matrix H_hat = compute_homography_ba(trimmed_matches);
    // Calculate inliers.
    std::vector<Match> inliers = model_inliers(H_hat, m, thresh);
    // Check if the inliers are better than the previous estimate.
    if (inliers.size() > best) {
      Hba = compute_homography_ba(inliers);
      best = inliers.size();
      if (best > cutoff) return Hba;
    }
  }
...
Image combine_images(const Image& a, const Image& b, const Matrix& Hba,
  ...
  // Loop over the part of the new canvas that we suspect that the new image
  // might map to. Can we only loop over a subset of the pixels ?

  for (int i = 0; i < c.w; i++) {
    for (int j = 0; j < c.h; j++) {
      // Reproject the point into b's co-ordinate frame.
      Point p(i + dx, j + dy);
      Point p_proj = project_point(Hba, p);
      // See if the projection falls within the image b.
      if (b.contains(p_proj.x, p_proj.y)) {
        for (int k = 0; k < c.c; k++) {
          // See if we need to blend with the pixels of a.
          if (c(i, j, k)) {
            c(i, j, k) =
                ablendcoeff * c(i, j, k) +
                (1 - ablendcoeff) * b.pixel_bilinear(p_proj.x, p_proj.y, k);
          } else {
            c(i, j, k) = b.pixel_bilinear(p_proj.x, p_proj.y, k);
          }
        }
      }
    }
...
Image panorama_image(const Image& a, const Image& b, float sigma,
                     int corner_method, float thresh, int window, int nms,
